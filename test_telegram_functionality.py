...
class TestMockTelegramClient:
    """Test the mock client for testing."""
    
    @pytest.mark.asyncio
    async def test_mock_client_simulation(self, test_config):
        """Test mock client message simulation."""
        from local_orchestrator_tray.telegram_client import MockTelegramClient
        ...
        with patch('subprocess.run') as mock_run:
            ...
            client = MockTelegramClient(test_config)
            
            # Test message with existing action
            toml_message = """
[hello]
message = "test"
"""
            
            response = await client.simulate_message(toml_message)
            assert "Action 'hello' completed" in response
            
            # Test message with non-existent action
            invalid_message = """
[nonexistent-action]
param = "value"
"""
            
            response = await client.simulate_message(invalid_message)
            assert "not found" in response
            assert "Available actions" in response
            
    def test_mock_client_status(self, test_config):
        """Test mock client status tracking."""
        from local_orchestrator_tray.telegram_client import MockTelegramClient
        
        client = MockTelegramClient(test_config)
        
        # Initial status
        assert "Mock mode" in client.get_connection_status()
        
        # Start client
        success = client.start_client()
        assert success
        assert "connected" in client.get_connection_status().lower()
        
        # Stop client
        client.stop_client()
        assert "disconnect" in client.get_connection_status().lower()
