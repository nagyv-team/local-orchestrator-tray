...
class TestSensitiveDataSanitization:
    ...
    def test_config_sanitization_in_string_representation(self, create_temp_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            config_str = str(client.config) if hasattr(
                client, 'config') else ''
    ...
    def test_telegram_api_errors_sanitized(self, create_temp_config, capture_logs):
        ...
        with patch('telegram_client.Update'), \
            ...
            mock_app = Mock()
            mock_app.initialize = AsyncMock(side_effect=Exception(
                "401 Unauthorized: Invalid bot token: 1111111111:AABBCCDDEEFFGGHHIIJJKKLLMMNNOOPPQQRRs"))
        ...
        with patch('telegram_client.Update'), \
            ...
            for payload in malicious_payloads:
                ...
                try:
                    ...
                    ), f"Should not mention injection in error: {e}"

                # Should not execute dangerous commands
                if mock_run.called:
                    executed_cmd = mock_run.call_args[0][0]
                    dangerous_patterns = [
                        'rm -rf', 'DROP TABLE', '../../', '<script>', 'alert(', 'passwd']
                    for pattern in dangerous_patterns:
                        assert pattern not in str(
                            executed_cmd), f"Dangerous pattern '{pattern}' in command: {executed_cmd}"
    ...
    def test_config_file_permissions_not_too_permissive(self, create_temp_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            try:
                ...
                pytest.fail(
                    "Should handle restrictive file permissions gracefully")
    ...
    def test_stack_traces_dont_expose_tokens(self, create_temp_config, capture_logs):
        ...
        with patch('telegram_client.Update'), \
            ...
            mock_run.side_effect = Exception(
                "Command failed with token: 5555555555:SecretTokenThatShouldNotLeakInStackTraces")
