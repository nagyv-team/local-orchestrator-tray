    ...
    async def test_intermittent_network_failure_resilience(self, create_temp_config, recovery_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            mock_run.return_value = Mock(
                stdout="network test", stderr="", returncode=0)
            ...
            for i in range(10):
                ...
                if i % 3 == 0:  # Every 3rd message has network issues
                    mock_message.reply_text.side_effect = ConnectionError(
...
class TestConfigurationErrorRecovery:
    ...
    def test_config_reload_after_validation_fix(self, create_temp_config):
        ...
        invalid_config = {
            'telegram': {'bot_token': 'PLACEHOLDER_TOKEN'},
            'actions': {}
        }
    ...
    def test_partial_config_corruption_recovery(self, create_temp_config):
        ...
        mixed_config = {
            ...
            'actions': {
                ...
                # Invalid - no command
                'invalid_action': {'missing_command': True},
    ...
    def test_config_file_disappearance_handling(self, create_temp_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            try:
                ...
                assert client.config_error is not None
                assert 'not found' in client.config_error.lower(
                ) or 'missing' in client.config_error.lower()
            except AttributeError:
                # reload_config method doesn't exist yet - expected in TDD
                pass
            except FileNotFoundError:
...
class TestRuntimeErrorRecovery:
    ...
    async def test_command_execution_failure_recovery(self, create_temp_config, recovery_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            execution_results = [
                Mock(stdout="", stderr="Command failed",
                     returncode=1),  # Failure
                Mock(stdout="Success", stderr="", returncode=0),  # Success
                Mock(stdout="", stderr="Another failure",
                     returncode=1),  # Failure
                Mock(stdout="Recovery success",
                     stderr="", returncode=0),  # Success
            ]
    ...
    def test_graceful_shutdown_and_restart(self, create_temp_config, recovery_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            def create_tracked_app():
                mock_app = Mock()
                mock_app.initialize = AsyncMock(
                    side_effect=lambda: startup_calls.append('initialize'))
                mock_app.start = AsyncMock(
                    side_effect=lambda: startup_calls.append('start'))
                mock_app.stop = AsyncMock(
                    side_effect=lambda: shutdown_calls.append('stop'))
                mock_app.shutdown = AsyncMock(
                    side_effect=lambda: shutdown_calls.append('shutdown'))
                mock_app.add_handler = Mock()

                mock_updater = Mock()
                mock_updater.start_polling = AsyncMock(
                    side_effect=lambda **kwargs: startup_calls.append('start_polling'))
                mock_updater.stop = AsyncMock(
                    side_effect=lambda: shutdown_calls.append('stop_polling'))
                mock_app.updater = mock_updater
            ...
            assert len(
                startup_calls) > 0, "Should have startup calls after restart"
    ...
    def test_ungraceful_termination_recovery(self, create_temp_config, recovery_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            def create_failing_app():
                ...
                mock_app.stop = AsyncMock(
                    side_effect=Exception("Ungraceful termination"))
                mock_app.shutdown = AsyncMock(
                    side_effect=Exception("Force shutdown"))

                mock_updater = Mock()
                mock_updater.start_polling = AsyncMock()
                mock_updater.stop = AsyncMock(
                    side_effect=Exception("Polling stop failed"))
                mock_app.updater = mock_updater
            ...
            try:
                ...
                pytest.fail(
                    f"Should handle ungraceful shutdown gracefully: {e}")
    ...
    def test_resource_cleanup_after_failures(self, create_temp_config, recovery_config):
        ...
        with patch('telegram_client.Update'), \
            ...
            def create_resource_tracking_app():
                ...
                mock_updater.start_polling = AsyncMock(
                    side_effect=track_start_polling)
            ...
            for cycle in range(3):
                # Start client
                client.start_client()
                assert len(allocated_resources) > len(
                    cleaned_resources), "Should allocate resources"
                ...
                assert len(
                    cleaned_resources) > 0, f"Should clean up some resources in cycle {cycle}"
        ...
        with patch('telegram_client.Update'), \
            ...
            for i, exception in enumerate(exceptions):
                ...
                try:
                    ...
                    mock_message.reply_text.assert_called_once()
                    reply_text = mock_message.reply_text.call_args[0][0]
                    assert any(word in reply_text.lower()
                               for word in ['error', 'failed', 'issue'])

                except Exception as e:
                    # Should not propagate exceptions to caller
                    pytest.fail(
                        f"Exception {type(exception).__name__} should be handled gracefully: {e}")
            ...
            mock_run.return_value = Mock(
                stdout="recovery test", stderr="", returncode=0)
